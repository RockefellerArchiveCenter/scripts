...
def makeSipDirectory(sip_location, sip_name):
    ''' Creates a SIP directory with necessary subdirectories.

    sip_location: target directory to package SIPs
    sip_name: name to be used in file name of SIP (e.g., an RFOD title)
    '''
    print("Making SIP directory...")
    sip_name = "sip_" + sip_name
    os.mkdir(os.path.join(sip_location, sip_name))
    sip_directory = os.path.join(sip_location, sip_name)
    os.mkdir(os.path.join(sip_directory, "logs"))
    os.mkdir(os.path.join(sip_directory, "metadata"))
    objectsDirectory = os.path.join(sip_directory, "objects")
...
def copyFiles(source, destination):
    '''
    Copies files from one directory (source) to a different directory (destination).
    Does not copy files ending in "bs.db" to avoid Thumbs.db files.
    Removes _me and _se from end of filename if present.
    '''
    print("Copying files from {} to {}...".format(source, destination))
    for f in os.listdir(source):
        if not f[-5:] == "bs.db":
            copy2(os.path.join(source, f), destination)
            if f[-7:-4] in ["_me", "_se"]:
                newName = f[:-7] + f[-4:]
                os.replace(os.path.join(destination, f),
                           os.path.join(destination, newName))


def createAspaceCsv(metadata, access):
    '''
    Creates the first column of an archivesspaceids.csv file
    '''
...
def findDate(date):
    '''
    Takes the last 4 digits between 1900 and 2000 in a string.
    '''
...
parser.add_argument(
    'sip_directory', help='Path to the directory where each SIP should be placed.')
parser.add_argument(
    'officers', help='Filepath to text file with officers (one per line).')
parser.add_argument(
    '-a', '--aspace', help='Option to create the first column of an archivesspaceids.csv file.', action='store_true')
parser.add_argument(
    '-p', '--premis', help='Option to a rights.csv file. Assumes creation year is in filename.', action='store_true')
...
for o in officers:
    ...
    for d in diaries:
        if d not in ['.DS_Store', 'Thumbs.db']:
            ...
            sourceService = os.path.join(
                officerDirectory, "TIFFs", "Master-Edited", d)
            ...
            print(str(int(elapsed_time / 60)) + " minutes, " +
                  str(int(elapsed_time % 60)) + " seconds elapsed")
